 ...
 *   Message on Complete
 */
function ntxuva_install_finished(&$install_state) {

  // Saving taxonomies for status:
  ntxuva_create_categories();
  // Saving taxonomies for category:
  ntxuva_create_status();
  // Creating example node:
  ntxuva_create_reports();
  // Activating blocks:
  ntxuva_build_blocks();
  // Deleting dummy entries:
  ntxuva_delete_dummies();
  // Create slider image files:
  ntxuva_create_pages();

  drupal_set_title(st('MTXUVA installation complete'));
  $messages = drupal_set_message();

  $output  = '<p>' . st('Congratulations, you installed ntxuva @drupal!', array('@drupal' => drupal_install_profile_distribution_name())) . '</p>';
...
function ntxuva_create_status() {
  ...
  $terms[0] = array(
    'Novo',
    'Categoria para novos casos',
    'cc0000',
    'pause',
  );
  $terms[1] = array(
    'Em progresso',
    'Categoria para os casos que aguardam resolução',
    'ff6600',
    'play',
  );
  $terms[3] = array(
    'Resolvido',
    'Categoria para casos resolvidos',
    '8fe83b',
    'checkmark',
  );
  $terms[4] = array(
    'Arquivado',
    'Categoria para casos arquivados',
    '5F9EA0',
    'drawer',
  );
 ...
 */
function ntxuva_create_categories() {
  ...
  // Define the terms.
  $terms[0] = array(
    'Contentor não recolhido',
    'O contentor não foi recolhido',
    '010',
    'contentornaorecolhido',
    '00008B',
    'remove',
  );
  $terms[1] = array(
    'Contentor a Arder',
    'O contentor foi incendiado',
    '011',
    'contentorarder',
    '5F9EA0',
    'fire',
  ...
  $terms[2] = array(
    ...
    'Situações em que o lixo se encontra fora do contentor',
    '012',
    'lixeira, lixonochao',
    '8B0000',
    'bug',
  ...
  $terms[3] = array(
    'Lixeira informal',
    'Situações em que o lixo se encontra amontoado em locais não predeterminados',
    '013',
    'lixeira, lixoamontoado',
    '006400',
    'spam',
  );
 ...
 * Create Reports.
 */
function ntxuva_create_reports() {
  ...
  $nodes[0] = array(
    ...
    '-25.94319334116242',
    '32.54931449890137',
    'Rua Aida Augusto,  Cidade de Maputo',
    'jjnf@communities.pt',
    'Contentor não recolhido',
    'Novo',
  ...
  $nodes[1] = array(
    ...
    '-25.934782520610856',
    '32.57419466972351',
    'Avenida de Angola, Cidade de Maputo',
    'jjnf@communities.pt',
    'Contentor a Arder',
    'Novo',
  ...
  $nodes[2] = array(
    ...
    '-25.941683148077356',
    '32.58596420288086',
    'Avenida Milagre Mabote, Cidade de Maputo',
    'jjnf@communities.pt',
    'Lixo espalhado no chão',
    'Resolvido',
    'flickr_thomasbrandt',
  ...
  foreach ($nodes as $node_data) {
    ...
    $image = file_get_contents('profiles/ntxuva/themes/mas/images/' . $node_data[8] . '.jpg');
...
function ntxuva_create_pages() {
  ...
  foreach ($nodes as $node_data) {
    ...
    $image = file_get_contents('profiles/ntxuva/themes/ntxuva/images/' . $node_data[2] . '.jpg');
    $file = file_save_data($image, 'public://' . $filename, FILE_EXISTS_RENAME);
    $node->field_image = array(LANGUAGE_NONE => array('0' => (array) $file));
 ...
 */
function ntxuva_build_blocks() {
  ntxuva_activate_block('views', 'report_log-report_log', 'content', 'ntxuva', 'node/*', '1', '5');
  ntxuva_activate_block('markaspot_logic', 'taxonomy_category', 'sidebar_second', 'ntxuva', 'map', '1', '0');
  ntxuva_activate_block('markaspot_logic', 'taxonomy_status', 'sidebar_second', 'ntxuva', 'map', '1', '0');
  ntxuva_activate_block('markaspot_logic', 'markaspot_map', 'sidebar_second', 'ntxuva', 'map', '1', '0');
  ntxuva_activate_block('system', 'navigation', 'sidebar_second', 'ntxuva', '<front>' . "\n" . 'node/7' . "\n" . 'node/8', '1', '0');
  ntxuva_activate_block('menu', 'menu-secondary-navigation', 'footer', 'ntxuva', 'admin' . "\n" . 'admin/*', '0', '0');
  ntxuva_activate_block('user', 'login', 'sidebar_second', 'ntxuva', '<front>' . "\n" . 'node/7' . "\n" . 'node/8', '1', '0');
  ntxuva_activate_block('markaspot_default_content', 'welcome', 'content', 'ntxuva', '<front>', '1', '0');
  ntxuva_activate_block('markaspot_stats', 'markaspot_stats', 'sidebar_second', 'ntxuva', '<front>' . "\n" . 'node/7' . "\n" . 'node/8', '1', '0');
  ntxuva_activate_block('comment', 'recent', 'sidebar_second', 'ntxuva', '<front>', '1', '0');
  ntxuva_activate_block('user', 'new', 'sidebar_second', 'ntxuva', '<front>', '1', '0');
}
/**
 * Activate blocks.
 */
function ntxuva_activate_block($module, $block, $region, $theme, $pages, $visibility, $weight) {
 ...
 */
function ntxuva_delete_dummies() {
  // Get the vocabulary ID.
  $vid = db_query("SELECT vid FROM {taxonomy_vocabulary} WHERE machine_name = 'status'")->fetchField();
